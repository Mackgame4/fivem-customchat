...
{
    const s = "<span>" + (str.replace(/\^([a-z0-9])/g, (str, color) => `</span><span class="color-${color}">`)) + "</span>";
	//const s = "<span>" + (str.replace(/\{(.+?)\}/g, (str, color) => `</span><span style="color:#${color};">`)) + "</span>";
    return s.replace(/<span[^>]*><\/span[^>]*>/g, '');
}

function colorize2(str)
{
    //const s = "<span>" + (str.replace(/\^([a-z0-9])/g, (str, color) => `</span><span class="color-${color}">`)) + "</span>";
	const s = "<span>" + (str.replace(/\{(.+?)\}/g, (str, color) => `</span><span style="color:#${color};">`)) + "</span>";
    return s.replace(/<span[^>]*><\/span[^>]*>/g, '');
...
$(function()
{
    ...
    window.addEventListener('message', function(event)
    {
        ...
        if (name != '')
        {
            nameStr = '<strong style="color: rgb(' + colorR + ', ' + colorG + ', ' + colorB + ')">[' + h + ':' + m + ':' + s + '] ' + name + ': </strong>';
        }
		else
		{
			nameStr = '<strong style="color: rgb(' + colorR + ', ' + colorG + ', ' + colorB + ')">['+h+':'+m+':'+s+']</strong> ';
		}
